...
public class ReviewReadServlet extends HttpServlet {
    ...
    private static final String FILE_PATH = "C:\\Users\\IQBAL\\OneDrive\\Desktop\\Project\\Data\\review_data.txt";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Selection sort by "rating" in descending order
            for (int i = 0; i < reviews.size() - 1; i++) {
                int maxIndex = i;
                for (int j = i + 1; j < reviews.size(); j++) {
                    int ratingJ = reviews.get(j).get("rating").getAsInt();
                    int ratingMax = reviews.get(maxIndex).get("rating").getAsInt();
                    if (ratingJ > ratingMax) {
                        maxIndex = j;
                    }
                }
                if (maxIndex != i) {
                    JsonObject temp = reviews.get(i);
                    reviews.set(i, reviews.get(maxIndex));
                    reviews.set(maxIndex, temp);
                }
            }
